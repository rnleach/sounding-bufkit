...
impl Indexes {
    pub fn parse(src: &str) -> Result<Indexes> {
        ...
        // value! The easy fix would be to replace head with src in all of the parse_f32 function
        ...
        use parse_util::parse_f32;

        let (show, head) = parse_f32(src, "SHOW").unwrap_or((-9999.0, src));
        let (lift, head) = parse_f32(head, "LIFT").unwrap_or((-9999.0, head));
        let (swet, head) = parse_f32(head, "SWET").unwrap_or((-9999.0, head));
        let (kinx, head) = parse_f32(head, "KINX").unwrap_or((-9999.0, head));
        let (lclp, head) = parse_f32(head, "LCLP").unwrap_or((-9999.0, head));
        let (pwat, head) = parse_f32(head, "PWAT").unwrap_or((-9999.0, head));
        let (totl, head) = parse_f32(head, "TOTL").unwrap_or((-9999.0, head));
        let (cape, head) = parse_f32(head, "CAPE").unwrap_or((-9999.0, head));
        let (lclt, head) = parse_f32(head, "LCLT").unwrap_or((-9999.0, head));
        let (cins, head) = parse_f32(head, "CINS").unwrap_or((-9999.0, head));
        let (eqlv, head) = parse_f32(head, "EQLV").unwrap_or((-9999.0, head));
        let (lfct, head) = parse_f32(head, "LFCT").unwrap_or((-9999.0, head));
        let (brch, _) = parse_f32(head, "BRCH").unwrap_or((-9999.0, head));
